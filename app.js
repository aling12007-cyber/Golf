(function(){
  ...
  function syncTabs(){ticking=false;var offset=window.scrollY+window.innerHeight*.22,index=0;sections.forEach(function(s,i){if(s&&s.offsetTop<=offset)index=i});if(index===current)return;current=index;tabs.forEach(function(a,i){a.classList.toggle('active',i===index)});var t=tabs[index];if(t)t.scrollIntoView({behavior:'smooth',block:'nearest',inline:'center'})}
  function onScroll(){if(!ticking){requestAnimationFrame(syncTabs);ticking=true}}
  window.addEventListener('scroll',onScroll,{passive:true});window.addEventListener('resize',onScroll);syncTabs();
})();